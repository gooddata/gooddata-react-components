...
describe("NumericInput", () => {
    ...
    ])("should %s the value from %p to %i when the %s button is clicked", (_, from, to, buttonGetter) => {
        const onChange = jest.fn();
    ...
    ])("should %s %i to %i on click with min %p and max %p", (_, from, to, min, max, buttonGetter) => {
        const onChange = jest.fn();
    ...
    ])("should %s the value from %p to %i when the %s arrow is pressed", (_, from, to, arrowCode) => {
        const onChange = jest.fn();
        const rendered = shallow(<NumericInput onChange={onChange} value={from} />);
    ...
    ])("should %s %i to %i on arrow press with min %p and max %p", (_, from, to, min, max, arrowCode) => {
        const onChange = jest.fn();
        const rendered = shallow(<NumericInput onChange={onChange} min={min} max={max} value={from} />);
