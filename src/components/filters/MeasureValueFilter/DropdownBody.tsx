...
class DropdownBodyWrapped extends React.PureComponent<IDropdownBodyProps, IDropdownBodyState> {
    ...
    private convertToRawValue = (value: IMeasureValueFilterValue): IMeasureValueFilterValue => {
        if (!value) {
            return value;
        }
        const rawValue: IMeasureValueFilterValue = value.value
            ? { value: value.value / 100 }
            : { from: value.from / 100, to: value.to / 100 };
        return rawValue;
    };

    private convertToPercentageValue = (value: IMeasureValueFilterValue): IMeasureValueFilterValue => {
        if (!value) {
            return value;
        }
        const rawValue: IMeasureValueFilterValue = value.value
