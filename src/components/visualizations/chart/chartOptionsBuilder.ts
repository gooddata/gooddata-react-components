...
function findParentMeasureIndex(afm: AFM.IAfm, measureItemIndex: number) {
...
export function getColorPalette(
    ...
    const paletteMeasures = range(0, measureGroup.items.length).map((measureItemIndex) => {
        ...
        const colorIndex = sourceMeasuresCounter % colorPalette.length;
        sourceMeasuresCounter++;
