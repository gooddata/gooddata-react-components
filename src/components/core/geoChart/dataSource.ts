...
function getLocationName(locationName: any): string {
    return locationName.attributeHeaderItem.name;
}

function getLocationLatLng(locationValue: any): number[] {
    const latlng = locationValue.attributeHeaderItem.name;
    const [lat, lng] = latlng.split(";").map((location: string): number => parseFloat(location));
    return [lng, lat];
}
...
function transformData(execution: any): IGeoDataSource {
    ...
    const [locationLatLng, locationName] = headerItems[0];
    ...
    const features = data.map(
        (dataItem: any, index: number): IGeoDataSourceFeature => {
            ...
            return {
                ...
                geometry: {
                    type: "Point",
                    coordinates: getLocationLatLng(locationLatLng[index]),
                },
                properties: {
                    City: getLocationName(locationName[index]),
